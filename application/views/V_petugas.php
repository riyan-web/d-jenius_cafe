...
<div class="box box-solid box-danger">
  <div class="box-header with-border">
    <h3 class="box-title"><i class="fa fa-book"></i> <b>Daftar Karyawan </b></h3>
  ...
  <div class="box-body">
    ...
    <table id="table_karyawan" class="table table-hover table-bordered table-striped">
      <thead>
        <tr>
          <th>No</th>
          <th>Nama karyawan</th>
  ...
  <div class="box-footer">
    Menampilkan daftar Karyawan, mengedit dan menghapus. klik tombol pada kolom pilihan.
  </div><!-- box-footer -->
</div>
<?= $modal_petugas?>
...
$(document).ready(function() {
    dataTable = $('#table_karyawan').DataTable( {
      "serverSide": true,
      "stateSave" : false,
      "bAutoWidth": true,
      "oLanguage": {
      ...
      "ajax":{
        url :"<?= base_url('petugas/petugas_list'); ?>",
        type: "post",
        error: function(){ 
          $(".my-grid-error").html("");
  ...
  function petugas_tambah()
  {
      ...
      $('#petugas').modal('show');
      $('.form-msg').html('');
      $('.modal-title').text('Tambah User Karyawan Baru'); 
  }
  ...
  function simpan()
{
    ...
    if(save_method == 'tambahPetugas') {
        url = "<?= site_url('petugas/petugas_tambah')?>";
    } else {
        url = "<?= site_url('petugas/petugas_proses_ubah')?>";
    }
...
function petugas_ubah(id)
{
    ...
    $('#petugas').modal('show'); 
    $('.form-msg').html('');
    $('.modal-title').text('Ubah Data User Karyawan');


    //Ajax Load data from ajax
    $.ajax({
        url : "<?= site_url('petugas/petugas_ubah')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
var kdnya;
  ...
  $(document).on("click", ".hapus-dataPetugas", function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?= base_url('petugas/petugas_hapus'); ?>",
      data: "id=" +idnya
    })
    .done(function(data) {
