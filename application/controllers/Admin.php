...
class Admin extends CI_Controller {
	...
	public function index() {
		...
		$data['modal_admin'] = show_my_modal('_modal/mdl_petugas', 'petugas', $data);
		$this->template->views('V_dataAdmin', $data);
    }

    public function admin_list() {
		$requestData	= $_REQUEST;
		$fetch			= $this->petugas->admin_data($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
	...
	public function petugas_proses_ubah() {
		...
		if($Namae->nama_lengkap != $this->input->post('nama_lengkap') && $cekNama > 0) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($result > 0) {
					$out['status'] = '';
					$out['msg'] = show_succ_msg('Data user Admin Berhasil diubah', '20px');
				} else {
					$out['status'] = '';
					$out['msg'] = show_succ_msg('Data user Admin Gagal diubah', '20px');
				}
			} else {
				$out['status'] = 'form';
				$out['msg'] = show_err_msg(validation_errors());
	...
	public function petugas_hapus() {
		...
		if ($result > 0) {
			echo show_succ_msg('Data user Admin Berhasil dihapus', '20px');
		} else {
			echo show_err_msg('Data user Admin Gagal dihapus', '20px');
		}
	}
}